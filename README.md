# AdiOS (v3.0 Beta: Sovereign Developer Studio, Online Package Manager, Visual File Explorer & 3D Spatial Computing)
...
- **Release Status**: `v3.0.0-beta (Sovereign Studio, Online Package Manager & Spatial Computing)`
- **Target Architecture**: RISC-V 32-bit (RV32IM + H-Extension)
- **Codebase Scale**: **50,000+ Lines of Code across 200 source files**
- **Verification Harness**: **56/56 Automated Subsystems Passing (100% Success) | 117/117 Unit Tests Passing**
- **Minimalist Modern Design**: **Four curated minimalist palettes (Nordic Slate, Monochrome Silver, Arctic Minimal, Emerald Code) with crisp hairline borders**
- **In-OS Code Studio**: **Multi-tab developer studio with line numbers, syntax token highlighting, execution runner sandbox, and preloaded 3D math & benchmark templates**
- **Online Package Manager**: **Asynchronous background PyPI package installer (`pip install`) with real-time log streaming and instant runtime library importing**
- **AdioFiles Visual Explorer**: **Dual-pane filesystem navigator with Places sidebar, extension-aware type badges, size formatting, and direct file launcher hooks**
- **3D Spatial Scene Studio**: **Interactive mouse-drag 3D model inspector with Octahedron, Hexagonal Prism, Monolith, and Pyramid geometric primitives**
...
- **Sovereign YouTube Player**: **Real Internet YouTube Video/Audio Streaming (640x360 @ 60 FPS HD, 32-Band EQ Visualizer, Sample-Accurate In-Memory WAV Seeking)**

<div align="center">
  <img src="docs/assets/adios_v3_beta_workstation.png" alt="AdiOS v3.0 Beta Sovereign Workstation 1280x720 HD" width="920"/>
  <p><em>Figure 1: Live AdiOS v3.0 Beta Workstation in Nordic Slate theme running in 1280x720 HD at 60 FPS with In-OS Code Studio (displaying 3D geometry matrix projection with line numbers and syntax highlighting), 3D Scene Studio (rendering rotating Octahedron with diffuse lighting), Desktop Icon Grid, and Top Taskbar with [THEME: NORDIC] Quick Toggle.</em></p>
</div>

<div align="center">
  <img src="docs/assets/adios_v3_beta_packages_explorer.png" alt="AdiOS Online Package Manager and File Explorer" width="920"/>
  <p><em>Figure 2: AdiOS Online Package Manager tab installing libraries directly from PyPI via asynchronous background pip runner with live log streaming, alongside AdioFiles dual-pane visual file explorer with Places sidebar and file badges.</em></p>
</div>

<div align="center">
  <img src="docs/assets/adios_v2_youtube_player.png" alt="AdiOS Sovereign YouTube Player 60 FPS HD" width="920"/>
  <p><em>Figure 3: Live Sovereign YouTube Player streaming real video at 640x360 @ 60 FPS HD with dancing 32-band audio spectrum EQ visualizer, real-time waveform oscilloscope, and sub-10ms hardware audio lip synchronization.</em></p>
</div>
...
## 2. What Is New in v3.0 Beta (Sovereign Studio & Spatial Computing)

AdiOS v3.0 Beta elevates the sovereign computing environment with developer tools, online package installation, and spatial 3D inspection:

### 1. Minimalist Modern Design System & Theme Engine (`desktop/theme.py`)
- Four curated, high-legibility minimalist palettes tailored for engineering focus:
  - **Nordic Slate**: Deep charcoal and slate with crisp ice-blue accents.
  - **Monochrome Silver**: Minimalist graphite and silver-white studio aesthetics.
  - **Arctic Minimal**: High-contrast daylight theme with clean charcoal ink typography.
  - **Emerald Code**: Subtle dark terminal styling with GitHub-style green highlights.
- Seamless 1-click theme switching via dedicated `[THEME: KEY]` quick toggle in the top taskbar.
- Refined window chrome featuring subtle hairline borders, 6px corner radiuses, and traffic-light control buttons.

### 2. In-OS AdiOS Code Studio IDE (`desktop/code_studio.py`)
- Complete developer studio inside an interactive desktop window.
- **Multi-Line Editor**: Line number gutter, active line highlight, syntax token coloring (keywords, builtins, numbers, strings, comments).
- **Execution Sandbox**: Live `[Run]` button capturing stdout/stderr with millisecond-accurate timing telemetry.
- **Preloaded Sovereign Templates**: Instant 1-click loading of 3D spatial geometry projection, Sieve of Eratosthenes benchmark, and HTTP networking scripts.

### 3. Integrated Online Package Manager (`pip install`)
- Live PyPI bridge built directly into Code Studio's `Packages (pip)` tab.
- Asynchronous non-blocking package installation using background subprocess threads.
- Real-time streamed terminal installation logs with instant module readiness in the code execution sandbox.
- Quick-install badges for popular libraries (`cowsay`, `requests`, `numpy`).

### 4. AdioFiles Visual File Explorer (`desktop/file_explorer.py`)
- Dual-pane filesystem manager:
  - **Places Sidebar**: Quick-access bookmarks to `Root`, `Scripts`, `Desktop`, `Graphics`, and `Tests`.
  - **File List Table**: Extension-aware badges (`[D]` for directory, `[F]` for file), human-readable formatted file sizes (`12.4 KB`, `1.2 MB`), and file types.
- **Application Launcher Hooks**: Double-clicking `.py`, `.ap`, `.s`, `.c`, `.txt`, `.md` files opens them directly inside Code Studio; media files trigger the media player.

### 5. Interactive 3D Spatial Scene Studio (`desktop/scene3d_studio.py`)
- Real-time 3D spatial inspector powered by the software rasterizer (`graphics/engine3d.py`).
- **Mouse Drag 3D Rotation**: Left-click anywhere on the canvas to rotate models smoothly in azimuth and elevation.
- **Geometric Primitives Library**: Cube, Temple Pyramid, Octahedron, Hexagonal Prism, and Sovereign Monolith (1:4:9 ratio).
- **Shading Modes**: Solid flat-shaded polygons with directional diffuse lighting and wireframe mesh inspection.
- **Guaranteed Viewport Containment**: Scissor-clipped rasterization preventing any overflow beyond window boundaries.

---

## 3. What Was New in v2.0 Stable
